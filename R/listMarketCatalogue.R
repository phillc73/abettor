...
listMarketCatalogue <- function(eventTypeIds, marketTypeCodes, maxResults = "200", fromDate = (format(Sys.time()-7200, "%Y-%m-%dT%TZ")), toDate = (format(Sys.time() + 86400, "%Y-%m-%dT%TZ")),
                                eventIDs=NULL,competitionIDs=NULL,marketIDs=NULL,marketCountries=NULL,venues=NULL,bspOnly=NULL,turnInPlayEnabled=NULL,inPlayOnly=NULL,marketBettingTypes=NULL,
                                withOrders=NULL,MarketSort=NULL,marketProjection=c("COMPETITION", "EVENT", "EVENT_TYPE", "RUNNER_DESCRIPTION", "RUNNER_METADATA", "MARKET_START_TIME"),textQuery=NULL,sslVerify = TRUE){
  
  options(stringsAsFactors=FALSE)
  listMarketCatalogueOps <- data.frame(jsonrpc = "2.0", method = "SportsAPING/v1.0/listMarketCatalogue", id = "1")
  
  listMarketCatalogueOps$params <- data.frame(filter = c(""), maxResults = c(maxResults))
  listMarketCatalogueOps$params$filter <- data.frame(marketStartTime = c(""))
  ...
  if(!is.null(competitionIDs)){
    listMarketCatalogueOps$params$filter$competitionIds <- list(competitionIDs)}
  ...
  listMarketCatalogueOps$params$filter$turnInPlayEnabled <- turnInPlayEnabled
  ...
  if(!is.null(marketBettingTypes)){
    listMarketCatalogueOps$params$filter$marketBettingTypes <- list(marketBettingTypes)}
  ...
  if(!is.null(marketProjection))  {
    listMarketCatalogueOps$params$marketProjection <- list(marketProjection)}
