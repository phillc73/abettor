#' Change the price of a set of unmatched bets \link{https://api.developer.betfair.com/services/webapps/docs/display/1smk3cen4v3lu3yomq5qye0ni/replaceOrders}
#'
#'
#'
#' \code{replaceOrders} changes the price of a specific set of unmatched bets from the same market.
#'
#' Important Info: This operation is logically a bulk cancel followed by a bulk place. The cancel is completed first then the new orders are placed. 
#' The new orders will be placed atomically in that they will all be placed or none will be placed. In the case where the new orders cannot be placed, the cancellations will not be rolled back.
#'
#' 
...
#'
#' @param marketId String. The market ID of the bets to be replaced. While many bets can be
#' updated in one call, they must be from the same market.
#'   
#' @param betID vector (strings). The bet IDs of the bets to be replaced- bet IDs
#' are displayed (called Ref) on the bet information on the right hand side of market page on the betfair desktop site.
#' 
#' @param newPrice vector (strings). The new price of the original bets (e.g "1.01", "2.22","10.0",etc). This parameter 
#' needs to be set of quoted doubles (see for more information on accepted price doubles (e.g. 2.02 is viable, while 10.02 is not))
#' 
...
#'
#'
#' @section Note on \code{replaceOrders}: Unlike some other functions that utilised data
#' frames, this function converts the input to a JSON-compatible format. The JSON output is then
